...
export function usePrefixPathWithLocale(path: string) {
  ...
  const url = new URL(location.pathname, 'https://example.com');
  const firstPathPart = url.pathname.split('/')[1]?.toLowerCase() ?? '';

  // Extract pathPrefix from current location
  const pathPrefix = /^[a-z]{2}-[a-z]{2}$/i.test(firstPathPart)
    ? '/' + firstPathPart
    : '';
...
export function useIsHomePath() {
  ...
  const pathPrefix = /^[a-z]{2}-[a-z]{2}$/i.test(firstPathPart)
    ? '/' + firstPathPart
    : '';
  const strippedPathname = pathname.replace(pathPrefix, '');
